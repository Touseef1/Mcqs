...
export default function McqsClientComponent({ 
  ...
  const [selectedPaper, setSelectedPaper] = useState<string | null>(initialPapers[0]?._id || null);
  ...
  const filteredMcqs = selectedPaper
    ? mcqs.filter((mcq) => mcq.paper?._id === selectedPaper)
    : mcqs;
  ...
  const selectedPaperName = papers.find(p => p._id === selectedPaper)?.name || '';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between border-b border-gray-200 pb-6 pt-8">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">
            PPSC Paper 
            {selectedPaper && (
              <span className="ml-2 text-blue-600">({selectedPaperName})</span>
            )}
          </h1>
            ...
            <div className="sticky top-20">
              ...
              <ul className="space-y-2">
                {papers.map((paper) => (
                  <li key={paper._id}>
                    <button
                      onClick={() => setSelectedPaper(paper._id)}
                      className={`block w-full text-left px-3 py-2 rounded-lg ${
                        selectedPaper === paper._id
                          ? 'bg-blue-100 text-blue-800'
                          : 'text-gray-700 hover:bg-gray-100'
            ...
            {filteredMcqs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="mt-1 text-gray-500">
                  {selectedPaper
                    ? `No questions available in ${selectedPaperName} paper.`
